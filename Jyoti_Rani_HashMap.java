...
public class Jyoti_Rani_HashMap {
...
class MyHashMap<K, V> implements MyMap<K, V> {
  ...
  public V put(K key, V value) {
    int k =0;
    int bucketIndex2 = hash(key.hashCode());
    while (get(key) != null) { // The key is already in the map

      LinkedList<Entry<K, V>> bucket = table[bucketIndex2];
      for (Entry<K, V> entry: bucket)
        if (entry.getKey().equals(key)) {
        ...
        bucketIndex2 += Math.pow(k++, 2);
			bucketIndex2 %= capacity;
    }
